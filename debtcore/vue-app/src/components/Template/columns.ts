...
export const columns: ColumnDef<Task>[] = [
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const language = languages.find((x) => x.value === languageValue.toLowerCase())

			if (!language) return null

			return h('div', { class: 'flex items-center' }, [h('span', {}, language.label)])
		}
	},
	{
		...
		cell: ({ row }) => {
			...
			const type = types.find((type) => type.value === typeValue.toLowerCase())

			if (!type) return null

			return h('div', { class: 'flex items-center' }, [h('span', {}, type.label)])
		}
	},
	{
		accessorKey: 'status',
		...
		cell: ({ row }) => {
			...
			const status = statuses.find((status) => status.value === statusValue?.toLowerCase())
