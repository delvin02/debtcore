...
var message = computed(() => {
  if (!previewMessage.body || !previewMessage.body.text || !previewMessage.body.example || !previewMessage.body.example.body_text || !previewMessage.body.example.body_text[0]) {
    return '';  
  }
        ...
        <div class="flex items-stretch gap-4 h-full">
          <div class="w-[300px] border-r-2 pr-4">
            ...
            <div class="h-full mt-2 w-[200px]">
              <Label variant="ghost" class="w-full justify-start">
                English
              </Label>
            </div>
          </div>
          <div class="w-[400px]">
            ...
            <div class="h-full">
              ...
              <div class="block mx-4 my-3 ">
                <div class="box-border inline-block w-[300px] my-2 p-1 bg-[#fff] relative rounded after:h-[31px] after:-left-3 after:absolute after:top-3 after:bg-whatsapp-chat-edge after:bg-no-repeat after:w-3 after:bg-contain after:content-['']">
                  <div v-if="previewMessage.head">
                    <iframe v-if="previewMessage.head.example?.header_handle" :src="previewMessage.head.example?.header_handle[0]" class="w-full h-64 rounded px-2" type="application/pdf"></iframe>
                  </div>
                  
                  <div class=" min-h-5 relative break-words p-1 rounded">
                    <p class="text-left text-black px-1 text-sm whitespace-pre-wrap" v-if="previewMessage.body" v-html="message"></p>
                    <div class="flex justify-between items-end" v-if="previewMessage.footer">
